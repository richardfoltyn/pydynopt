...
def _action_info(ps, i_state, ix_state=0):
    act_dummy, _, _, inext, pnext = ps.actions(i_state)
...
def pfi(ps, tol=1e-8, n_accel=30, maxiter=1000, transm=True):
    ...
    p_d = ps.death_rate
    nstates_exo = ps.nstates_exo
    nstates_end = ps.nstates_end
    beta = ps.discount

    v_beg = np.zeros((nstates_exo, nstates_end), dtype=np.float64)
    v_end = np.empty_like(v_beg)
    v_old = np.empty_like(v_beg)
    util_cont = np.empty_like(v_beg)
    util_liq = np.empty_like(v_beg)
    ...
    pnext_x = np.zeros_like((nstates_exo, nstates_exo), dtype=np.float64)
    ...
    opt_choice = np.empty((nstates_exo, nstates_end, dim_a), dtype=dtype_a)
    ...
    # unflexible.
    inext_end = np.empty((nstates_exo, nstates_end, dim_inext), dtype=np.uint32)
    pnext_end = np.empty((nstates_exo, nstates_end, dim_inext),
                         dtype=np.float64)
    ...
    for it in range(maxiter):
        ...
        for ix in range(nstates_exo):
            v_end[ix] = beta * pnext_x[ix].dot(v_old[ix])

            if it % n_accel == 0:
                for i in range(nstates_end):
                    acts, u_cont, u_liq, inext, pnext = ps.actions(i, ix)

                    assert np.all(np.abs(pnext.sum(axis=-1) - 1) < 1e-12)

                    v_try = p_d * u_liq + (1 - p_d) * (
                        u_cont + np.sum(v_end[ix, inext] * pnext, axis=-1))
                    ...
                    util_liq[ix, i] = u_liq[j_max]

        # Update value fun, possibly without re-optimization.
        for ix in range(nstates_exo):
            v_ix = v_end[ix, inext_end[ix]]
            v_beg[ix] = p_d * util_liq[ix] + (1 - p_d) * (
                util_cont[ix] + np.sum(v_ix * pnext_end[ix], axis=-1))
        ...
        if it % n_accel == 0:
            ...
            if dv < tol:
                if transm:
                    ...
                    arr_size = nstates_exo * nstates_end * dim_inext * \
                               np.sum(pnext_x[0] > 0)
                    probs = np.empty((arr_size, ), dtype=float)
                    indptr = np.empty((ps.nstates + 1, ), dtype=int)
                    ...
                    for i in range(ps.nstates_end):
                        for ix in range(ps.nstates_exo):
                            ...
                            iexo = vstack(ind2sub(inext_x[ix], dims=ps.grid_shape_exo))
                            ...
                            if indptr[last_idx] + len(inext) > curr_size:
                                new_size = (curr_size * ps.nstates) // last_idx
                    ...
                    max_idx = indptr[ps.nstates]
                    transm = csr_matrix(
                        (probs[:max_idx], indices[:max_idx], indptr), shape=(ps.nstates, ps.nstates))

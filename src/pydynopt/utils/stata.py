"""
This work is licensed under CC BY 4.0,
https://creativecommons.org/licenses/by/4.0/

Author: Richard Foltyn
"""

from collections.abc import Mapping, Sequence
import contextlib
import glob
import logging
import os
from pathlib import Path
import shutil
import sys
import tempfile
from typing import Any


def _check_stata_works(path_exe: str | Path) -> bool:
    """
    Check that the given path to Stata executable works, i.e. Stata can actually run
    stuff. Even an existing executable might not work if the corresponding license is
    not valid.
    E.g., stata-mp is present on Linux even irrespective of whether the user has a
    license or not, and it will refuse to run if no license was obtained.

    Parameters
    ----------
    path_exe : str or Path

    Returns
    -------
    bool
    """
    dtmp = Path(tempfile.mkdtemp())

    # Result file
    out_file = dtmp / 'output.csv'
    # Do file
    do_file = dtmp / 'batch.do'

    contents = f"""
        set obs 1
        gen var = 1
        export delimited using `"{out_file.as_posix()}"', replace novarnames
    """

    with do_file.open('wt') as f:
        f.write(contents)

    # Batch mode: use /e on Windows, otherwise Stata shows a dialog box that the
    # script completed.
    batch = '/e' if sys.platform.lower().startswith('win') else '-b'

    # Execute Stata
    import subprocess

    subprocess.run([str(path_exe), batch, str(do_file)], cwd=str(dtmp), check=False)

    # Read output file
    works = False
    if out_file.is_file():
        with out_file.open('rt') as f:
            contents = f.readline().strip()
            works = contents == '1'

    # Delete temp directory
    shutil.rmtree(dtmp)

    return works


def find_stata(dirs: str | Path | Sequence[str | Path] | None = None) -> Path:
    """
    Try to locate a working Stata executable in the system PATH, in known locations
    or in the optionally given additional directories.

    Parameters
    ----------
    dirs : str, Path, or Sequence of str/Path, optional

    Returns
    -------
    Path
        Absolute path of Stata executable, if found.
    """
    is_win = sys.platform.lower().startswith('win')

    # List of candidate executable files
    if is_win:
        exes = [
            f'Stata{v}{bits}.exe'
            for bits in ('-64', '')
            for v in ('MP', 'SE', 'IC', '')
        ]
    else:
        exes = [f'stata{v}' for v in ('-mp', '-se', '')]

    for exe in exes:
        if path_exe := shutil.which(exe):
            path_exe_path = Path(path_exe)
            # Try to execute so code since exe might be present, but user might not
            # have the license for MP
            if _check_stata_works(path_exe_path):
                return path_exe_path

    # executable was not found in PATH, try other candidate directories
    user_dirs = dirs

    if is_win:
        sysdrive = os.environ.get('SYSTEMDRIVE', 'C:')
        if sysdrive.endswith(':'):
            sysdrive += '\\'
        dirs_list = [
            os.environ.get('PROGRAMFILES', 'C:\\Program Files'),
            os.environ.get('PROGRAMFILES(X86)', 'C:\\Program Files (x86)'),
            sysdrive,
        ]
        dirs_list = [os.path.join(d, 'Stata*') for d in dirs_list if d]
    else:
        dirs_list = ['/opt/stata/**', '/usr/local/**']

    # Append any additional candidate strings
    if isinstance(user_dirs, (str, Path)):
        dirs_list.insert(0, str(user_dirs))
    elif isinstance(user_dirs, Sequence):
        dirs_list = [str(d) for d in user_dirs] + dirs_list

    for exe in exes:
        for d in dirs_list:
            pattern = os.path.join(d, exe)

            candidates = glob.glob(pattern)
            for path_exe in candidates:
                path_exe_path = Path(path_exe)
                if _check_stata_works(path_exe_path):
                    return path_exe_path

    raise FileNotFoundError('Stata executable not found.')


def run_stata(
    do_file: str | Path,
    exe: str | Path | None = None,
    conf_file: str | Path | None = None,
    macros: Mapping[str, str] | None = None,
    global_macros: bool = False,
    cwd: str | Path | None = None,
    env: Mapping[str, str] | None = None,
    delete_log: bool = True,
    **kwargs: Any,
) -> int:
    """
    Run Stata in batch mode, executing the given do-file. Optionally
    creates a configuration do-file with given macros and adds
    given variables to the environment.

    Parameters
    ----------
    do_file : str or Path
        Do-file to run
    exe : str or Path, optional
        Stata executable. If not an absolute path, tries to locate
        the executable in the PATH, and if that fails, checks standard
        locations on Linux.
    conf_file : str or Path, optional
        If present, creates a configuration do-file at the given location
        and populates it with variables from `macros`.
    macros : Mapping, optional
        If present, writes key-value pairs as macros to do-file
        given in `conf_file`.
    global_macros : bool
        If True, write `macros` as global macros, and as local macros otherwise.
    cwd : str or Path, optional
        If present, sets the current working directory for the Stata process.
    env : Mapping, optional
        If present, augments the environment for the Stata process with
        the given name-value pairs.
    delete_log : bool
        If true, delete the log file that Stata automatically generates in batch mode.
    kwargs
        Keyword arguments passed to subprocess.run()

    Returns
    -------
    int
        Stata return code
    """
    import subprocess

    logger = logging.getLogger('IO')

    # Convert paths
    do_file = Path(do_file)

    # --- Locate Stata executable ---

    path_exe = Path(exe) if exe else find_stata()

    logger.info(f'Using Stata executable {path_exe}')

    # --- Create (optional) config file ---

    if conf_file and macros:
        conf_file = Path(conf_file)
        logger.info(f'Creating config file {conf_file}')

        prefix = 'global' if global_macros else 'local'

        with conf_file.open('wt') as f:
            for key, value in macros.items():
                print(f'{prefix} {key} = {value}', file=f)

    # --- Run Stata ---

    # Merge inherited environment with additional variables (note that
    # if env is passed, subprocess.run() replaces the inherited env variables)
    env_all = dict(os.environ)
    if env:
        env_all.update(env)

    kw = kwargs.copy()
    cwd_path = Path(cwd) if cwd else Path.cwd()
    kw.update({'cwd': cwd_path, 'env': env_all})

    # Batch mode: use /e on Windows, otherwise Stata shows a dialog box that the
    # script completed.
    batch = '/e' if sys.platform.lower().startswith('win') else '-b'

    # Execute Stata. check=True makes subprocess.run raise on a nonzero exit code.
    # Pop the caller's setting to pass it explicitly; otherwise handle it below.
    check = kw.pop('check', False)
    result = subprocess.run([str(path_exe), batch, str(do_file)], check=check, **kw)

    if result.returncode:
        raise RuntimeWarning(f'Stata exited with return code {result.returncode}')

    # Delete config file
    if conf_file and macros:
        conf_file = Path(conf_file)
        # On Windows this might not be possible because some other process still
        # is accessing the file
        with contextlib.suppress(PermissionError):
            conf_file.unlink()

    # Delete the log file that Stata automatically generates, assuming that logging
    # is explicitly handled within the do-file
    if delete_log:
        base = do_file.with_suffix('')
        fn = cwd_path / f'{base.name}.log'
        if fn.is_file():
            with contextlib.suppress(PermissionError):
                fn.unlink()

    return result.returncode
